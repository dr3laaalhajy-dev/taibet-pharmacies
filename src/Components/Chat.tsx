  ...
  useEffect(() => {
    ...
    newSocket.on('session_ended', (data: { conversation_id: number, closed_by: string, is_patient: boolean }) => {
      if (activeChat && activeChat.conversation_id == data.conversation_id) {
        ...
        // 🟢 التعديل الجديد: إظهار التقييم للعميل عندما يغلق الموظف المحادثة من عنده
        if (user.role !== 'admin' && user.role !== 'customer_service') {
          setRatingStaffId(activeChat.other_user_id); // حفظ رقم الموظف
          setShowRatingModal(true); // إظهار نافذة النجوم
        } else {
          // إذا كان المستخدم هو الموظف، قم بمسح الشاشة فوراً
          setActiveChat(null);
          setMessages([]);
          fetchConversations();
        }
