...
import { Send, ArrowRight, User as UserIcon, Clock, Check, CheckCheck, MessageSquare, Search, X, PowerOff } from 'lucide-react';
...
export const Chat = ({ user, lang, onClose, targetUserId = null, onSessionEnded }: { user: UserType, lang: string, onClose?: () => void, targetUserId?: number | null, onSessionEnded?: (doctorId: number) => void }) => {
  ...
  useEffect(() => {
    ...
    // 🟢 الاستماع لأمر إنهاء الجلسة من الطرف الآخر
    newSocket.on('session_ended', (data: { conversation_id: number, closed_by: string, is_patient: boolean }) => {
      if (activeChat && activeChat.conversation_id == data.conversation_id) {
        ...
        // إذا كان المستخدم هو المريض، نظهر له نافذة التقييم
        if (data.is_patient && onSessionEnded && activeChat.other_user_id) {
          onClose && onClose(); // إغلاق الشات
          onSessionEnded(activeChat.other_user_id); // فتح التقييم
        }
      } else {
        fetchConversations(); // تحديث القائمة لإخفاء المحادثة المغلقة
      }
    });
  ...
  const endConversation = async () => {
    ...
    try {
      ...
      // إذا كان المريض هو من أنهى المحادثة، افتح له نافذة التقييم
      if (user.role === 'patient' && onSessionEnded) {
        onClose && onClose();
  ...
  return (
    <div className="flex h-[85vh] md:h-[650px] w-full bg-slate-50 dark:bg-slate-950 rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 transition-colors">
      ...
      <div className={`w-full md:w-1/3 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col transition-all z-20 ${activeChat ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex justify-between items-center transition-colors">
          <h2 className="text-xl font-black text-slate-800 dark:text-white flex items-center gap-2"><MessageSquare className="text-blue-600 dark:text-blue-400"/> {lang === 'ar' ? 'الرسائل النشطة' : 'Active Chats'}</h2>
          {onClose && <button onClick={onClose} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-full transition-colors"><X size={24}/></button>}
        </div>
            ...
            filteredConversations.map(conv => (
              <div key={conv.conversation_id} onClick={() => openChat(conv)} className={`p-4 border-b border-slate-50 dark:border-slate-800 cursor-pointer flex items-center gap-4 hover:bg-blue-50 dark:hover:bg-slate-800 transition-colors ${String(activeChat?.other_user_id) === String(conv.other_user_id) ? 'bg-blue-50/50 dark:bg-slate-800 border-l-4 border-l-blue-600' : ''}`}>
                <div className="relative shrink-0">
                  {conv.other_user_image ? <img src={conv.other_user_image} className="w-14 h-14 rounded-full object-cover border border-slate-200 dark:border-slate-700 shadow-sm" /> : <div className="w-14 h-14 bg-blue-100 dark:bg-slate-700 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center font-bold text-xl shadow-sm">{conv.other_user_name?.[0]}</div>}
      ...
      <div className={`w-full md:w-2/3 bg-slate-50 dark:bg-slate-950 flex flex-col transition-colors relative z-10 ${!activeChat ? 'hidden md:flex' : 'flex'}`}>
        ...
        {activeChat ? (
          <>
            <div className="p-4 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center z-10 shadow-sm transition-colors">
              <div className="flex items-center gap-4">
                <button onClick={() => setActiveChat(null)} className="md:hidden p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full"><ArrowRight size={24} className={lang === 'ar' ? '' : 'rotate-180'} /></button>
                <div className="relative">
                  {activeChat.other_user_image ? <img src={activeChat.other_user_image} className="w-12 h-12 rounded-full object-cover border border-slate-100 dark:border-slate-700" /> : <div className="w-12 h-12 bg-blue-100 dark:bg-slate-800 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center font-bold text-xl">{activeChat.other_user_name?.[0]}</div>}
                ...
                <div>
                  <h3 className="font-extrabold text-slate-900 dark:text-white text-lg leading-tight">{activeChat.other_user_name}</h3>
                  <span className="text-xs text-slate-500 font-medium capitalize">{activeChat.other_user_role === 'patient' ? (lang==='ar'?'مريض':'Patient') : (lang==='ar'?'فريق الدعم / طبيب':'Staff')}</span>
                </div>
              </div>

              {/* 🟢 زر إنهاء المحادثة الجديد */}
              {activeChat.conversation_id > 0 && (
                <button onClick={endConversation} disabled={isEnding} className="flex items-center gap-2 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-3 py-1.5 rounded-lg text-sm font-bold hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors disabled:opacity-50 shadow-sm border border-red-100 dark:border-red-800">
          ...
          <div className="flex-1 flex flex-col items-center justify-center z-10 text-slate-400 relative p-8 text-center">
            <div className="w-32 h-32 bg-white dark:bg-slate-900 rounded-full shadow-sm flex items-center justify-center mb-6">
              <MessageSquare size={56} className="text-blue-100 dark:text-slate-700" />
            </div>
            <h3 className="text-2xl font-black text-slate-700 dark:text-slate-300 mb-2">{lang === 'ar' ? 'مركز الرسائل المؤقتة' : 'Live Chat Center'}</h3>
            <p className="text-base text-slate-500 max-w-sm leading-relaxed">{lang === 'ar' ? 'يتم حذف الرسائل تلقائياً بعد الضغط على إنهاء المحادثة لضمان خصوصيتك.' : 'Messages are cleared when the session ends.'}</p>
          </div>
        )}
      </div>
    </div>
