...
export const Chat = ({ user, lang, onClose, targetUserId = null, onSessionEnded }: { user: UserType, lang: string, onClose?: () => void, targetUserId?: number | null, onSessionEnded?: (doctorId: number) => void }) => {
  ...
  // 🟢 حالات التقييم ودعم العملاء
  const [showRatingModal, setShowRatingModal] = useState(false);
  const [ratingStaffId, setRatingStaffId] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    newSocket.on('session_ended', (data: { conversation_id: number, closed_by: string, is_patient: boolean }) => {
      if (activeChat && activeChat.conversation_id == data.conversation_id) {
        ...
        const otherId = activeChat.other_user_id;

        // إظهار التقييم فقط إذا كانت محادثة دعم فني، والمستخدم ليس موظفاً
        if (isSupport && user.role !== 'customer_service' && user.role !== 'admin' && otherId > 0) {
          setRatingStaffId(otherId);
          ...
          if (data.is_patient && onSessionEnded && otherId && !isSupport) {
            if (onClose) onClose(); 
            onSessionEnded(otherId); 
          }
        }
      } else {
        fetchConversations(); 
  ...
  const endConversation = async () => {
    ...
    try {
      ...
      if (isSupport && user.role !== 'customer_service' && user.role !== 'admin' && otherId > 0) {
        ...
        // للمرضى: تقييم الطبيب العادي إن وجد
        if (!isSupport && user.role === 'patient' && onSessionEnded) {
          if (onClose) onClose();
          onSessionEnded(otherId);
        }
      }
  ...
  return (
    <div className="flex h-[85vh] md:h-[650px] w-full bg-slate-50 dark:bg-slate-950 rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 transition-colors">
        ...
        <div className="p-0 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex flex-col transition-colors">
          <div className="p-5 flex justify-between items-center">
            ...
            {onClose && (
              <button 
                onClick={() => {
                  if (activeChat?.type === 'support' && user.role !== 'customer_service' && user.role !== 'admin' && activeChat.other_user_id > 0) {
                    setRatingStaffId(activeChat.other_user_id);
                    setShowRatingModal(true);
                  } else {
                    if (onClose) onClose();
                  }
      ...
      {/* 🟢 شاشة تقييم الموظف (ستظهر الآن فقط لمحادثات الدعم الفني المغلقة!) */}
      <AnimatePresence>
        {showRatingModal && (
          <RatingModal
