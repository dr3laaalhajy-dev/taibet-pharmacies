          ...
          >
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 bg-blue-600 text-white rounded-2xl flex items-center justify-center font-bold text-2xl overflow-hidden shadow-inner">
                {staff?.profile_picture ? <img src={staff.profile_picture} className="w-full h-full object-cover" /> : (staff?.name?.[0] || 'م')}
              </div>
              <div>
            ...
            <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden shadow-2xl">
              <div className="p-6 border-b dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center font-bold">{selectedStaff?.name?.[0] || 'م'}</div>
 ...
 const openPatientHistory = async (patientId: number, patientName: string) => {
  ...
  >
    ...
    <span>{lang === 'ar' ? 'آراء العملاء' : 'Customer Reviews'}</span>
        ...
        <div className="hidden md:block p-4 border-t border-slate-100 dark:border-slate-800 mt-auto"><div className="flex items-center gap-3 px-4 py-3 mb-2"><div className="w-8 h-8 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center text-slate-500 dark:text-slate-400 font-bold shrink-0">{user?.name?.[0] || 'U'}</div><div className="flex-1 min-w-0"><p className="text-sm font-bold text-slate-900 dark:text-white truncate">{user?.name || 'User'}</p><p className="text-xs text-slate-500 dark:text-slate-400 capitalize">{user?.role === 'admin' ? (t?.admin || 'مدير') : (user?.role === 'dentist' ? (lang === 'ar' ? 'طبيب أسنان' : 'Dentist') : (user?.role === 'doctor' ? (t?.doctor || 'طبيب') : (user?.role === 'pharmacist' ? (t?.pharmacist || 'صيدلي') : 'مريض')))}</p></div></div><button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors"><LogOut size={18} /> {t?.logout || 'تسجيل الخروج'}</button></div>
      </div>
                    ...
                    <div key={req.conversation_id} className="bg-white dark:bg-slate-900 p-6 rounded-3xl border-2 border-indigo-50 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
                      ...
                      <div className="flex items-center gap-4 mb-6 relative z-10">
                        <div className="w-14 h-14 bg-indigo-100 dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 rounded-full flex items-center justify-center font-bold text-xl overflow-hidden shadow-sm">
                          {req?.profile_picture ? <img src={req.profile_picture} className="w-full h-full object-cover" /> : (req?.patient_name?.[0] || 'م')}
                        </div>
                        <div>
                   ...
                   <div className="max-h-48 overflow-y-auto space-y-2 pr-2">
                     {users.filter(u => (u.role === 'doctor' || u.role === 'dentist') && (u?.name || '').includes(doctorSearch)).map(doc => (
                        <button key={doc.id} type="button" onClick={() => setTargetDoctorId(doc.id)} className={`w-full text-right p-3 rounded-xl border flex items-center gap-3 transition-colors ${targetDoctorId === doc.id ? 'bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400 font-bold' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>
                           <div className="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center shrink-0 font-bold">{doc?.name?. [0]|| 'ط'}</div>
