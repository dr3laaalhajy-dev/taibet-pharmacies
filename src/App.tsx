          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white p-6 md:p-8 rounded-3xl shadow-2xl w-full max-w-sm">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-slate-900">{lang === 'ar' ? 'شحن رصيد المحفظة' : 'Top-up Wallet'}</h3>
              ...
              <form onSubmit={submitWalletRequest}>
                <label className="block text-sm font-bold text-slate-500 mb-2">{lang === 'ar' ? 'أدخل المبلغ الذي تود شحنه:' : 'Enter amount:'}</label>
