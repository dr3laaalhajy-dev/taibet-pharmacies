...
app.get('/api/public/doctors', async (req: any, res: any) => { try { res.json((await pool.query("SELECT id, name, email, role, phone, specialty, consultation_price, about, faqs, profile_picture FROM users WHERE role IN ('doctor', 'dentist') AND is_active = true AND show_in_directory = true")).rows); } catch (err: any) { res.status(500).json({ error: err.message }); } });
...
app.post('/api/public/orders', async (req: any, res: any) => {
  ...
  if (payment_method === 'wallet') {
    ...
    try {
      ...
      // 🟢 إصلاح TypeScript لفك التشفير
      const balance = parseFloat((await pool.query('SELECT wallet_balance FROM users WHERE id = $1', [(decoded as any).id])).rows[0].wallet_balance);
      if (balance < parseFloat(total_price)) return res.status(400).json({ error: 'رصيد غير كافٍ.' });
      await pool.query('UPDATE users SET wallet_balance = wallet_balance - $1 WHERE id = $2', [total_price, (decoded as any).id]);
