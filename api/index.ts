...
io.on('connection', (socket) => {
  socket.on('register', (userId) => { if(userId) userSockets.set(parseInt(userId), socket.id); });
...
// 🟢 2. إيقاف حظر الاتصالات الخارجية في مكتبة الأمان helmet
app.use(helmet({ crossOriginResourcePolicy: false }));
...
const initDB = async () => {
  try {
    ...
    // 🟢 السطر السحري لحل مشكلة users_role_check
    ...
    // 🟢 تعديل جدول المحادثات لدعم نظام الجلسات المؤقتة
...
// ==========================================
// 🚀 نظام الجلسات ودعم العملاء (Ticketing System)
// ==========================================

// 1. طلب التحدث مع خدمة العملاء (من المريض)
app.post('/api/chat/support/request', authenticateToken, async (req: any, res: any) => {
  try {
    const patientId = req.user.id;
    // هل يوجد طلب معلق أو جلسة نشطة بالفعل؟
    ...
    if (existing.rows.length > 0) {
      ...
      // إذا كانت هناك محادثة قديمة مغلقة، قم بتحديثها لطلب جديد، أو أنشئ واحدة جديدة
      ...
      if (closed.rows.length > 0) {
        ...
        // حيلة للـ Unique Constraint: نستخدم قيمة Null في user2_id
    ...
    // إرسال إشعار لجميع موظفي خدمة العملاء المتصلين
...
// 2. جلب الطلبات المعلقة (للوحة تحكم موظفي خدمة العملاء)
...
// 3. قبول طلب خدمة العملاء (أول موظف يضغط قبول)
app.post('/api/chat/support/accept/:id', authenticateToken, async (req: any, res: any) => {
  if (req.user.role !== 'customer_service' && req.user.role !== 'admin') return res.status(403).json({ error: 'ممنوع' });
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    // Lock Row (FOR UPDATE) لمنع موظف آخر من القبول في نفس اللحظة
    ...
    // إعلام المريض أن الموظف دخل الشات
    const patientSocket = userSockets.get(conv.rows[0].user1_id);
    if (patientSocket) io.to(patientSocket).emit('support_accepted', { conversation_id: req.params.id, agent_name: req.user.name });
    
    // إخفاء الطلب من شاشات باقي الموظفين
...
// 4. إنهاء المحادثة المؤقتة وحذف الرسائل (للطبيب أو خدمة العملاء أو المريض)
app.post('/api/chat/end/:id', authenticateToken, async (req: any, res: any) => {
  try {
    const convId = req.params.id;
    console.log(`[نظام الشات] محاولة إنهاء المحادثة رقم: ${convId} من قبل المستخدم: ${req.user.id}`);
    
    const conv = await pool.query('SELECT * FROM conversations WHERE id = $1', [convId]);
    if (conv.rows.length === 0) {
        console.log(`[نظام الشات] المحادثة رقم ${convId} غير موجودة في قاعدة البيانات.`);
        // غيرناها لـ 400 لكي لا يخفيها الفرونت إند
        return res.status(400).json({ error: 'المحادثة غير موجودة في قاعدة البيانات!' });
    }
    ...
    if (currentUserId !== u1 && currentUserId !== u2 && req.user.role !== 'admin') {
      console.log(`[نظام الشات] المستخدم ${currentUserId} يحاول إغلاق محادثة لا يملكها.`);
      return res.status(400).json({ error: 'مرفوض: أنت لست طرفاً في هذه المحادثة' });
    }

    // تفريغ الرسائل وإغلاق الجلسة
    ...
    // تحديد المريض لإظهار التقييم له
    let patientId = u1; 
    if (type !== 'support') {
      patientId = req.user.role === 'patient' ? currentUserId : (currentUserId === u1 ? u2 : u1);
    }

    // إرسال Socket للطرفين لإغلاق الشاشة (استخدمنا Number لضمان التطابق)
    ...
    console.log(`[نظام الشات] تم إغلاق المحادثة رقم ${convId} بنجاح.`);
...
// ==========================================
// باقي مسارات الدردشة الأساسية (محدثة لدعم الجلسات)
// ==========================================

app.get('/api/chat/conversations', authenticateToken, async (req: any, res: any) => {
  try {
    const userId = parseInt(req.user.id);
    // جلب المحادثات النشطة فقط (Active)
    const query = `
...
app.get('/api/chat/messages/:otherUserId', authenticateToken, async (req: any, res: any) => {
  try {
    ...
    // جلب المحادثة المباشرة النشطة فقط
...
app.post('/api/chat/messages', authenticateToken, async (req: any, res: any) => {
  try {
    ...
    // إعادة فتح الجلسة إذا كانت مغلقة عند بدء الشات من جديد
    let conv = await pool.query(`SELECT id, status FROM conversations WHERE user1_id = $1 AND user2_id = $2 AND type = 'direct'`, [user1, user2]);
    if (conv.rows.length === 0) {
    ...
    try {
      ...
      if (receiverData.rows.length > 0 && receiverData.rows[0].fcm_token) {
        await sendPushNotification(receiverData.rows[0].fcm_token, `رسالة جديدة من ${senderData.rows[0]?.name || 'تطبيق طيبة'}`, content);
      }
    } catch (notifError) {}
...
// ==========================================
// باقي المسارات (السجل الطبي، الوصفات، الصيدليات، المستخدمين...) بقيت كما هي لديك بالضبط!
// ==========================================

app.get('/api/medical-records/:patientId', authenticateToken, async (req: any, res: any) => { try { res.json((await pool.query('SELECT * FROM medical_records WHERE patient_id = $1', [req.params.patientId])).rows[0] || {}); } catch(err: any) { res.status(500).json({ error: err.message }); } });
...
app.post('/api/public/orders', async (req: any, res: any) => { const { pharmacy_id, customer_name, customer_phone, items, total_price, payment_method } = req.body; const client = await pool.connect(); try { await client.query('BEGIN'); let buyerId = null; if (payment_method === 'wallet') { const token = req.cookies.token; if (!token) throw new Error('AuthRequired'); buyerId = (jwt.verify(token, JWT_SECRET) as any).id; const userRes = await client.query('SELECT wallet_balance FROM users WHERE id = $1 FOR UPDATE', [buyerId]); if (parseFloat(userRes.rows[0].wallet_balance) < parseFloat(total_price)) throw new Error('InsufficientBalance'); await client.query('UPDATE users SET wallet_balance = wallet_balance - $1 WHERE id = $2', [total_price, buyerId]); } else { const token = req.cookies.token; if (token) { try { buyerId = (jwt.verify(token, JWT_SECRET) as any).id; } catch(e){} } } for (const item of items) { const productCheck = await client.query('UPDATE products SET quantity = quantity - $1 WHERE id = $2 AND quantity >= $1 RETURNING id', [item.qty, item.product_id]); if (productCheck.rows.length === 0) throw new Error(`ProductOutStock`); } const orderRes = await client.query('INSERT INTO orders (pharmacy_id, customer_name, customer_phone, items, total_price, user_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id', [pharmacy_id, customer_name, customer_phone, JSON.stringify(items), total_price, buyerId]); const pharmacyRes = await client.query('SELECT doctor_id FROM pharmacies WHERE id = $1', [pharmacy_id]); if (pharmacyRes.rows[0]?.doctor_id) { await client.query('INSERT INTO notifications (user_id, title, message) VALUES ($1, $2, $3)', [pharmacyRes.rows[0].doctor_id, '🛒 طلب جديد!', `طلب جديد (#${orderRes.rows[0].id})`]); } await client.query('COMMIT'); res.json({ success: true }); } catch (err: any) { await client.query('ROLLBACK'); if(err.message === 'AuthRequired') return res.status(401).json({ error: 'سجل دخول' }); if(err.message === 'InsufficientBalance') return res.status(400).json({ error: 'رصيد غير كافٍ' }); if(err.message === 'ProductOutStock') return res.status(400).json({ error: 'نفدت الكمية' }); res.status(500).json({ error: err.message }); } finally { client.release(); } });
