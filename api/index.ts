...
// 🟢 تهيئة قاعدة البيانات وإضافة عمود الإخفاء/الإظهار
const initDB = async () => {
  try {
    ...
    try { await pool.query(`ALTER TABLE users ADD COLUMN show_in_directory BOOLEAN DEFAULT true;`); } catch(e){} // 🟢 عمود جديد للإخفاء
...
const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => { if (err) return res.status(403).json({ error: 'ممنوع' }); req.user = user; next(); });
};
...
// 🟢 لا نعرض إلا الأطباء الذين يملكون show_in_directory = true
app.get('/api/public/doctors', async (req, res) => { try { res.json((await pool.query("SELECT id, name, email, role, phone, specialty, consultation_price, about, faqs, profile_picture FROM users WHERE role IN ('doctor', 'dentist') AND is_active = true AND show_in_directory = true")).rows); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/public/doctors/:id', async (req, res) => { try { const doctor = (await pool.query('SELECT id, name, email, role, phone, notes, specialty, consultation_price, about, faqs, profile_picture FROM users WHERE id = $1', [req.params.id])).rows[0]; if (!doctor) return res.status(404).json({ error: 'User not found' }); const facilities = (await pool.query('SELECT id, name, type, address, phone, specialty, services, consultation_fee, waiting_time, working_hours, whatsapp_phone, image_url FROM pharmacies WHERE doctor_id = $1', [doctor.id])).rows; res.json({ ...doctor, facilities }); } catch (err) { res.status(500).json({ error: err.message }); } });

app.get('/api/public/settings', async (req, res) => { try { res.json((await pool.query("SELECT value FROM settings WHERE key = 'footer'")).rows[0]?.value || {}); } catch (err) { res.status(500).json({ error: err.message }); } });
app.get('/api/public/products', async (req, res) => { try { res.json((await pool.query(`SELECT p.*, ph.name as pharmacy_name, ph.whatsapp_phone FROM products p JOIN pharmacies ph ON p.pharmacy_id = ph.id WHERE ph.is_ecommerce_enabled = true ORDER BY p.id DESC`)).rows); } catch (err) { res.status(500).json({ error: err.message }); } });
...
app.post('/api/auth/register', async (req, res) => {
  ...
  } catch (err) { res.status(err.code === '23505' ? 400 : 500).json({ error: err.code === '23505' ? 'البريد مستخدم!' : err.message }); }
});
app.post('/api/auth/logout', (req, res) => { res.clearCookie('token'); res.json({ message: 'تم تسجيل الخروج' }); });
app.get('/api/auth/me', authenticateToken, async (req, res) => { res.json({ user: (await pool.query('SELECT id, email, role, name, wallet_balance FROM users WHERE id = $1', [req.user.id])).rows[0] }); });
...
// 🟢 --- مسار تحديث بيانات الطبيب (حفظ الإخفاء والسعر) ---
app.post('/api/doctor/update-profile', authenticateToken, async (req, res) => {
  const { specialty, consultation_price, about, faqs, show_in_directory, user_id } = req.body;
  ...
  try {
    ...
    await pool.query(
      `UPDATE users SET specialty = $1, consultation_price = $2, about = $3, faqs = $4, show_in_directory = $5 WHERE id = $6`,
      [specialty, consultation_price, about, faqsString, show_in_directory, targetId]
    );
...
app.post('/api/wallet/request', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true });
  } catch(err) { res.status(500).json({ error: 'السبب الحقيقي: ' + err.message }); }
});

app.get('/api/admin/wallet-requests', authenticateToken, async (req, res) => {
  if (!(await isSuperAdmin(req.user.email))) return res.status(403).json({ error: 'ممنوع' });
  try { res.json((await pool.query("SELECT * FROM wallet_requests ORDER BY id DESC")).rows); } catch(err) { res.status(500).json({ error: err.message }); }
});

app.patch('/api/admin/wallet-requests/:id', authenticateToken, async (req, res) => {
  if (!(await isSuperAdmin(req.user.email))) return res.status(403).json({ error: 'ممنوع' });
...
app.post('/api/admin/wallet/:id', authenticateToken, async (req, res) => {
  if (!(await isSuperAdmin(req.user.email))) return res.status(403).json({ error: 'ممنوع' });
  try { await pool.query('UPDATE users SET wallet_balance = wallet_balance + $1 WHERE id = $2', [req.body.amount, req.params.id]); res.json({ success: true }); } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.patch('/api/admin/users/:id/approve', authenticateToken, async (req, res) => { if (req.user.role !== 'admin') return res.status(403).json({ error: 'ممنوع' }); await pool.query('UPDATE users SET is_active = TRUE WHERE id = $1', [req.params.id]); res.json({ success: true }); });
...
app.delete('/api/admin/users/:id', authenticateToken, async (req, res) => { if (req.user.role !== 'admin') return res.status(403).json({ error: 'ممنوع' }); await pool.query('DELETE FROM users WHERE id = $1', [req.params.id]); res.json({ success: true }); });
app.put('/api/admin/settings', authenticateToken, async (req, res) => { if (req.user.role !== 'admin' || !(await isSuperAdmin(req.user.email))) return res.status(403).json({ error: 'ممنوع' }); await pool.query("INSERT INTO settings (key, value) VALUES ('footer', $1) ON CONFLICT (key) DO UPDATE SET value = $1", [req.body]); res.json({ success: true }); });
app.post('/api/admin/generate-key', authenticateToken, async (req, res) => { if (!(await isSuperAdmin(req.user.email))) return res.status(403).json({ error: 'ممنوع' }); const newKey = Math.random().toString(36).substring(2, 10).toUpperCase(); await pool.query('INSERT INTO activation_keys (key) VALUES ($1)', [newKey]); res.json({ key: newKey }); });

// --- مسارات غرفة السوبر آدمن ---
app.get('/api/admin/super-admins', authenticateToken, async (req, res) => {
  if (!(await isSuperAdmin(req.user.email))) return res.status(403).json({ error: 'ممنوع' });
  const result = await pool.query('SELECT email FROM super_admins');
  res.json(result.rows.map(row => row.email));
});

app.post('/api/admin/super-admins', authenticateToken, async (req, res) => {
  if (!(await isSuperAdmin(req.user.email))) return res.status(403).json({ error: 'ممنوع' });
...
app.delete('/api/admin/super-admins/:email', authenticateToken, async (req, res) => {
  if (!(await isSuperAdmin(req.user.email))) return res.status(403).json({ error: 'ممنوع' });
